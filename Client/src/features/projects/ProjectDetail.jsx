...
export const ProjectDetail = () => {
  ...
  useEffect(() => {
    const fetchProject = async () => {
      setLoading(true);
      try {
        const pData = await mockService.getProjectById(id || 'proj-501');
        const tData = await mockService.getTasks();
        const aData = await mockService.getActivities(id || 'proj-501');

        setProject(pData);
        setTasks(tData.filter((t) => t.projectId === (id || 'proj-501')));
        setActivities(aData);
      } catch (e) {
        console.error(e);
      } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white border border-[#EEF1FA] p-6 rounded-[24px] shadow-[0_10px_30px_rgba(0,0,0,0.04)]">
        <div>
          ...
          <p className="text-xs text-[#8A8FA3] mt-1.5 font-mono">
            Client: <span className="font-semibold text-[#16181D]">{project.clientName}</span> | Project Manager: {project.managerName}
          </p>
        </div>
      ...
      {/* SIGNATURE FEATURE: SALES HANDOVER RECEIPT */}
      {project.handoverReceipt && (
        <div className="p-6 rounded-[24px] border border-[#3B5BFD]/30 bg-[#3B5BFD]/5 relative overflow-hidden shadow-xs space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 text-xs font-mono">
            <div>
              <span className="text-[#8A8FA3] block text-[10px] uppercase font-sans">
                Source Opportunity
              </span>
              <span className="font-bold text-[#16181D]">
                {project.handoverReceipt.sourceOpportunity}
              </span>
            </div>
            ...
            <div>
              ...
              <span className="font-bold text-[#3B5BFD]">
                {project.handoverReceipt.acceptedQuotation}
              </span>
            </div>
            ...
            <div>
              ...
              <span className="font-bold text-[#16181D]">
                {project.handoverReceipt.client}
              </span>
            </div>
            ...
            <div>
              ...
              <span className="font-bold text-sm text-[#3B5BFD]">
                {formatCurrency(project.handoverReceipt.commercialValue)}
              </span>
            </div>
          </div>
                ...
                tasks.map((task) => (
                  <div key={task.id} className="p-4 rounded-2xl bg-[#EEF1FA]/40 border border-[#EEF1FA] flex items-center justify-between text-xs">
                    <div>
                      ...
                      <div className="text-[11px] text-[#8A8FA3] font-mono">Assignee: {task.assigneeName}</div>
                    </div>
